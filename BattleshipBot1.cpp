...
#define MY_SHIP	SHIPTYPE_FRIGATE	//my ship type
...
{
	CONST int Flag = 2808; // Create a localised variable for my flag, separate from the standard flag variable so it is easy to switch between the two if necessary.
...
int nearest_enemy()
{
	int i;
	int nearest_ship = 0;
	int nearsetDistance = 200;
	//Loop round enemies to find the closest
	for (i = 0; i < number_of_enemies; i++)
	{
		int currentDistance = calculate_Ship_Distances(i);
...
void tactics()
{
	...
	int enemyToShoot = nearest_enemy();
